// Code generated by workspace-cli-generator. DO NOT EDIT.

package vuln

import (
	"fmt"
	"context"

	"github.com/chaitin/workspace-cli/products/cloudwalker/client"
	"github.com/spf13/cobra"
)

var createWhitelistParams CreateWhitelistParams

var CreateWhitelistCmd = &cobra.Command{
	Use:   "create_whitelist",
	Short: "生成白名单规则",
	Long:  `生成白名单规则`,
	Run: func(cmd *cobra.Command, args []string) {
		cli := client.GetClient()
		var result map[string]interface{}
		err := cli.Call(context.Background(), "VulnService.CreateWhitelist", createWhitelistParams, &result)
		if err != nil {
			cmd.PrintErrln("Error:", err)
			return
		}
		fmt.Fprintln(cmd.OutOrStdout(), client.PrintResult(result))
	},
}

func init() {
		CreateWhitelistCmd.Flags().StringSliceVar(&createWhitelistParams.AccessComplexity, "access-complexity", nil, "攻击复杂度")
		CreateWhitelistCmd.Flags().StringSliceVar(&createWhitelistParams.AccessVector, "access-vector", nil, "利用特征")
		CreateWhitelistCmd.Flags().BoolVar(&createWhitelistParams.Accessible, "accessible", false, "是否可访问")
		CreateWhitelistCmd.Flags().StringSliceVar(&createWhitelistParams.AppCategory, "app-category", nil, "软件类型")
		CreateWhitelistCmd.Flags().StringSliceVar(&createWhitelistParams.AppProduct, "app-product", nil, "影响软件")
		CreateWhitelistCmd.Flags().StringSliceVar(&createWhitelistParams.AppVersion, "app-version", nil, "软件版本号")
		CreateWhitelistCmd.Flags().StringSliceVar(&createWhitelistParams.Authentication, "authentication", nil, "攻击认证")
		CreateWhitelistCmd.Flags().StringSliceVar(&createWhitelistParams.Availability, "availability", nil, "可用性影响")
		CreateWhitelistCmd.Flags().StringSliceVar(&createWhitelistParams.Bid, "bid", nil, "BID 编号")
		CreateWhitelistCmd.Flags().StringSliceVar(&createWhitelistParams.Cnnvd, "cnnvd", nil, "CNNVD 编号")
		CreateWhitelistCmd.Flags().StringSliceVar(&createWhitelistParams.Cnvd, "cnvd", nil, "CNVD 编号")
		CreateWhitelistCmd.Flags().StringSliceVar(&createWhitelistParams.Comment, "comment", nil, "用户自定义备注")
		CreateWhitelistCmd.Flags().StringSliceVar(&createWhitelistParams.Confidentiality, "confidentiality", nil, "机密性影响")
		CreateWhitelistCmd.Flags().StringSliceVar(&createWhitelistParams.CreatedAt, "created-at", nil, "创建时间")
		// custom_attr is complex type []map[string]interface{}, use JSON string
		var customAttrJSON string
		CreateWhitelistCmd.Flags().StringVar(&customAttrJSON, "custom-attr", "", "主机业务属性 (JSON, e.g. [{\"attr_name\": \"负责人\", \"attr_value\": [\"David\"]}])")
		CreateWhitelistCmd.Flags().StringSliceVar(&createWhitelistParams.Cve, "cve", nil, "CVE 编号")
		CreateWhitelistCmd.Flags().StringSliceVar(&createWhitelistParams.CvssScore, "cvss-score", nil, "CVSS 分数")
		CreateWhitelistCmd.Flags().StringSliceVar(&createWhitelistParams.Cwe, "cwe", nil, "CWE 编号")
		CreateWhitelistCmd.Flags().Float64SliceVar(&createWhitelistParams.Gids, "gids", nil, "业务组 ID")
		CreateWhitelistCmd.Flags().StringSliceVar(&createWhitelistParams.GroupName, "group-name", nil, "业务组名")
		CreateWhitelistCmd.Flags().BoolVar(&createWhitelistParams.HasExp, "has-exp", false, "是否有 EXP")
		CreateWhitelistCmd.Flags().StringSliceVar(&createWhitelistParams.HostComment, "host-comment", nil, "主机备注")
		CreateWhitelistCmd.Flags().Float64SliceVar(&createWhitelistParams.HostId, "host-id", nil, "主机 ID")
		CreateWhitelistCmd.Flags().StringSliceVar(&createWhitelistParams.HostIp, "host-ip", nil, "主机 IP")
		CreateWhitelistCmd.Flags().StringSliceVar(&createWhitelistParams.HostName, "host-name", nil, "主机名称")
		CreateWhitelistCmd.Flags().StringSliceVar(&createWhitelistParams.HostState, "host-state", nil, "主机状态")
		CreateWhitelistCmd.Flags().StringSliceVar(&createWhitelistParams.HostTags, "host-tags", nil, "主机标签")
		CreateWhitelistCmd.Flags().Float64SliceVar(&createWhitelistParams.Id, "id", nil, "ID 用来在任务完成后按id筛选")
		CreateWhitelistCmd.Flags().StringSliceVar(&createWhitelistParams.Integrity, "integrity", nil, "完整性影响")
		CreateWhitelistCmd.Flags().BoolVar(&createWhitelistParams.IsKev, "is-kev", false, "是否是已知被利用漏洞")
		CreateWhitelistCmd.Flags().BoolVar(&createWhitelistParams.IsRansomware, "is-ransomware", false, " 是勒索软件攻击漏洞")
		CreateWhitelistCmd.Flags().IntSliceVar(&createWhitelistParams.Level, "level", nil, "漏洞级别")
		CreateWhitelistCmd.Flags().StringSliceVar(&createWhitelistParams.Name, "name", nil, "漏洞名称")
		CreateWhitelistCmd.Flags().Float64SliceVar(&createWhitelistParams.Oid, "oid", nil, "机构 ID")
		CreateWhitelistCmd.Flags().Float64SliceVar(&createWhitelistParams.PlanId, "plan-id", nil, "任务 id")
		CreateWhitelistCmd.Flags().StringSliceVar(&createWhitelistParams.PublishDate, "publish-date", nil, "漏洞发布时间")
		// select is complex type []map[string]interface{}, use JSON string
		var selectJSON string
		CreateWhitelistCmd.Flags().StringVar(&selectJSON, "select", "", "select (JSON, e.g. [{\"app_product\": \"windows_nt\", \"app_vendor\": \"null\", \"host_id\": 111, \"id\": 120, \"vuln_id\": 111}])")
		CreateWhitelistCmd.Flags().BoolVar(&createWhitelistParams.SelectAll, "select-all", false, "select_all")
		CreateWhitelistCmd.Flags().IntSliceVar(&createWhitelistParams.State, "state", nil, "事件状态(1-有风险，2-已忽略，3-已处理)")
		CreateWhitelistCmd.Flags().StringSliceVar(&createWhitelistParams.Tags, "tags", nil, "漏洞标签")
		CreateWhitelistCmd.Flags().Float64SliceVar(&createWhitelistParams.Type, "type", nil, "漏洞类型")
		CreateWhitelistCmd.Flags().StringSliceVar(&createWhitelistParams.UpdatedAt, "updated-at", nil, "更新时间")
		CreateWhitelistCmd.Flags().StringSliceVar(&createWhitelistParams.WhiteRuleId, "white-rule-id", nil, "白名单规则 ID")
		// whitelist is object type, use JSON string
		var whitelistJSON string
		CreateWhitelistCmd.Flags().StringVar(&whitelistJSON, "whitelist", "", "whitelist (JSON, e.g. {\"agent_range\": [1, 2, 3], \"builtin_id\": \"1\", \"business_group_range\": [1, 2, 3], \"...\": \"...\"})")
}


// CreateWhitelistParams 请求参数
type CreateWhitelistParams struct {
	AccessComplexity []string `json:"access_complexity"` // 攻击复杂度
	AccessVector []string `json:"access_vector"` // 利用特征
	Accessible bool `json:"accessible"` // 是否可访问
	AppCategory []string `json:"app_category"` // 软件类型
	AppProduct []string `json:"app_product"` // 影响软件
	AppVersion []string `json:"app_version"` // 软件版本号
	Authentication []string `json:"authentication"` // 攻击认证
	Availability []string `json:"availability"` // 可用性影响
	Bid []string `json:"bid"` // BID 编号
	Cnnvd []string `json:"cnnvd"` // CNNVD 编号
	Cnvd []string `json:"cnvd"` // CNVD 编号
	Comment []string `json:"comment"` // 用户自定义备注
	Confidentiality []string `json:"confidentiality"` // 机密性影响
	CreatedAt []string `json:"created_at"` // 创建时间
	CustomAttr []map[string]interface{} `json:"custom_attr"` // 主机业务属性
	Cve []string `json:"cve"` // CVE 编号
	CvssScore []string `json:"cvss_score"` // CVSS 分数
	Cwe []string `json:"cwe"` // CWE 编号
	Gids []float64 `json:"gids"` // 业务组 ID
	GroupName []string `json:"group_name"` // 业务组名
	HasExp bool `json:"has_exp"` // 是否有 EXP
	HostComment []string `json:"host_comment"` // 主机备注
	HostId []float64 `json:"host_id"` // 主机 ID
	HostIp []string `json:"host_ip"` // 主机 IP
	HostName []string `json:"host_name"` // 主机名称
	HostState []string `json:"host_state"` // 主机状态
	HostTags []string `json:"host_tags"` // 主机标签
	Id []float64 `json:"id"` // ID 用来在任务完成后按id筛选
	Integrity []string `json:"integrity"` // 完整性影响
	IsKev bool `json:"is_kev"` // 是否是已知被利用漏洞
	IsRansomware bool `json:"is_ransomware"` //  是勒索软件攻击漏洞
	Level []int `json:"level"` // 漏洞级别
	Name []string `json:"name"` // 漏洞名称
	Oid []float64 `json:"oid"` // 机构 ID
	PlanId []float64 `json:"plan_id"` // 任务 id
	PublishDate []string `json:"publish_date"` // 漏洞发布时间
	Select []map[string]interface{} `json:"select"` // select
	SelectAll bool `json:"select_all"` // select_all
	State []int `json:"state"` // 事件状态(1-有风险，2-已忽略，3-已处理)
	Tags []string `json:"tags"` // 漏洞标签
	Type []float64 `json:"type"` // 漏洞类型
	UpdatedAt []string `json:"updated_at"` // 更新时间
	WhiteRuleId []string `json:"white_rule_id"` // 白名单规则 ID
	Whitelist map[string]interface{} `json:"whitelist"` // whitelist
}

